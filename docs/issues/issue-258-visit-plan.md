...
| D-3  | **（2026-09-26 変更）「＋ 予定を組む」の日付の初期値は今日**（実機確認でのオーナー判断。「次の土曜」は理由が伝わらなかった。`nextSaturday` は残すが画面では使わない）。以下は変更前: **「次の土曜日」= 今日より後の最初の土曜日**。今日が土曜なら **7日後**（試作 `(6 - day + 7) % 7 \|\| 7`）。端末のローカル日付で数え、`toLocalDateString`（`src/utils/localDate.ts`）で `YYYY-MM-DD` にする。`toISOString()` は使わない（Issue #204）。例: 2026-09-26（土）→ 2026-10-03、2026-09-28（月）→ 2026-10-03、2026-10-02（金）→ 2026-10-03                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
- ドロワーの見出し: 左「{M月D日（曜）}の予定 ▾」、右「長押しで並べ替え」
- 提案した直後だけバナー「✦ 近い順・受付の早い順に並べました。長押しで入れ替えられます」
...
- [ ] AC-34: 京都5社を選んで「順番を決める」を押すと、ドロワーの行が上から 八坂神社・建仁寺・清水寺・三十三間堂・伏見稲荷大社 の順で、時刻 `9:00`〜`12:05`、区間の文字「徒歩 約9分」「徒歩 約17分」「電車などで移動・約1.6km」「電車などで移動・約3.3km」、バナー「✦ 近い順・受付の早い順に並べました。長押しで入れ替えられます」が出る
