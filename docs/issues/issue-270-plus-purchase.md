...
| M-2 | Claude（MCP）    | H-4 の後               | RevenueCat の iOS アプリ（bundle `com.goshuin.app`）に App Store の商品 `com.goshuin.app.plus` を足し、entitlement `plus` と offering `plus`（current。M-1 で `default` の見本と合わなかったので `plus` を作った）の `$rc_lifetime` に付ける                                                  |
