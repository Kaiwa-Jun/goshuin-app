...
スワイプはスクリーンリーダー利用時と自動テストの双方で扱いにくいため、**ページ送りはタップだけでも完結する**ように設計する。スワイプとスナップ物理の検証は N 群（native-only）に置く。なお Expo Web ではログイン済み状態に到達できずめくり表示自体が描画されない（後述「未解決事項」）。
...
1. **覗いている隣ページのタップで送る（S-6）** — スワイプ以外の送り手段。アクセシビリティ改善であり、Maestro からもページ送りを操作できるようになる
...
## 追補1: グリッドの並び順を古い順に確定（2026-08-09・実機確認でのフィードバック）

実機確認で「グリッドは左上が一番古くて、右へ・下へ進むほど新しいほうがよい」という指摘があり、そのとおりに変更した（`fix/gallery-grid-oldest-first`）。

**なぜグリッドだけ反転させなかったか**: グリッド・めくり・拡大表示（`ImageGalleryModal`）は `displayStamps` の同じインデックスを共有している。グリッドの描画順だけ反転すると、グリッドから開いた拡大表示のスワイプ方向がグリッドと逆になる。そのため**基準の並び自体を昇順に揃えた**。

- `useGalleryStamps` は日付順で**昇順**を返す（API は `visited_at DESC` なので反転する）
- `GoshuinchoFlipView` は渡された配列をそのまま使う（内部の `.reverse()` を削除）。「1ページ目 = 先頭 = 最も古い / 末尾の白紙が一番新しい側」という対応は不変
- `webPreview.web.ts` の fixture も昇順
- スポット順は従来どおり `localeCompare('ja')` のまま

**⚠️ 意図的にそうしている。「新しい順に戻す」変更を入れないこと。** 記録が増えると最新を見るのに一番下までスクロールすることになるトレードオフは認識したうえで、**御朱印帳らしさを優先する判断をユーザーが下した**（2026-08-09）。検討して見送った代替案は ①開いたとき最下部（最新）から始める ②「日付順 ▼」のトグルに古い順／新しい順を足す の2つ。件数が増えて実際に不便になったら再検討する。
