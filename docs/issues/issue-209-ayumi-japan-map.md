...
- [x] **E-1**: 地図の順序が **全国 → 寄る → ピン → 色づく**。寄り終わるまで今回の県は**記録する前の濃さ**のままで（初めての県なら `empty`、2回目以降なら1段下）、
...
| E-18 の残置理由が一次資料と食い違う（仕様書 §5 は朱で光ると書いてある）               | ✅ **仕様書 §5 の方を実装に合わせた**。朱で光らせると地図の色に「枚数」以外の意味が混ざる（①§0 で外したのと同じ理由）。寄り終わるまで**記録する前の濃さ**で出し、ピンと同時に1段濃くする形にした |
...
- 点滅の代わりに「**記録する前の濃さ → 1段濃くなる**」で瞬間を出している。ピンが落ちるのと
