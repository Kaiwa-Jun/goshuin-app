...
- [x] **E-1**: 地図の順序が **全国 → 寄る → ピン → 色づく**。寄り終わるまで今回の県は**記録する前の濃さ**のままで（初めての県なら `empty`、2回目以降なら記録前の段）、
...
### Evaluator の2巡目で出た指摘（対応済み）

| 指摘                                                                                  | 対応                                                                                                                                                      |
| ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **E-17 の数え上げが逆向き**。最終値が先に出て、寄り終わった瞬間に戻って数え直していた | ✅ 数え始めの値から出す。**地図を出さない経路では合図が来ない**ので、そのときは最終値のまま出す（これも取りこぼしていた）                                 |
| E-18 の残置理由が一次資料と食い違う（仕様書 §5 は朱で光ると書いてある）               | ✅ **仕様書 §5 の方を実装に合わせた**。理由は下記「朱の点滅を入れない理由」を参照。寄り終わるまで**記録する前の濃さ**で出し、ピンと同時に濃くなる形にした |
| H-5: `ui-design.md` §4.8 に `Issue #208` が残っている                                 | ✅ #209 に訂正                                                                                                                                            |
| I-6: Maestro フローが無い                                                             | ✅ **フローは追加しない**に変更（下記）                                                                                                                   |
| E-6 のテストに鹿児島が無く、「県の上」を県の範囲で見ていない                          | ✅ 沖縄・鹿児島・北海道・東京の4県で、**その県の画面上の範囲**にピンの先が入ることを見る                                                                  |
| F-3 にテストが無い                                                                    | ✅ `onRevealed` → 上の数字・凡例の配線をテストで固定                                                                                                      |
| E-20 の影が `stamp-image` ではなく枠側                                                | ✅ `Image` に影は乗らないので枠側が正。`stamp-frame` に testID を付けてトークン由来であることを見る                                                       |
| `RecentVisits` が影と余白を直値で書いている                                           | ✅ `shadows.md` と名前付き定数へ                                                                                                                          |

### 朱の点滅を入れない理由（E-18）と、食い違ったときの決め方

仕様書 §5 と承認デザインは**どちらも朱の点滅を書いていた**が、入れていない。理由を正確に書く。

- **実機で確認したのは点滅の無い版で、それで良いと判断された**（2026-09-21）。試作の点滅は
  実機に載る前の段階のもので、**最終的な合意は実機の側にある**
- 点滅の代わりに「**記録する前の濃さ → 濃くなる**」で瞬間を出している。ピンが落ちるのと
  合わせて、その県に何かが起きたことは十分に伝わる
- ⚠️ **「1段濃くなる」とは書かない。** まとめて4枚登録すれば2段またぐこともある

⚠️ **「地図の色に意味を混ぜない（①§0）」を理由にはしない。** あちらは次の訪問まで残り続ける
**状態の色**で、点滅は 0.5秒で元に戻る**一過性の演出**。別物なので、同じ理屈で片付けるのは
一般化しすぎだった（Evaluator の指摘で訂正）。

⚠️ **帰属の精度について**: 実機で確認してもらったのは「寄りとピン」で、**濃さの移り変わり
そのものを名指しで見てもらったわけではない**（点滅は試作の HTML にしか無く、アプリには一度も
入っていないので、実機で見たものが点滅なしだったことは確かではある）。

**食い違ったときの決め方**: 仕様書・承認デザイン・実装の3つが食い違ったら、
**実機で見て合意したものが正**。仕様書とデザインの方を実装に合わせて直す。
この節より上の「承認デザインに追い越されている記述」も、同じ理由で承認デザインを正としている
（あのときは実機がまだ無く、デザインがいちばん新しい合意だった）。

### I-6 の変更（Maestro フローは追加しない）

あゆみ画面は**ログイン必須**で、`smoke.yaml` に載せられない（CI にも実機の認証情報を置けない）。
#208 の契約書は同じ理由で「Maestro の新規フローは追加しない」としており、こちらを踏襲する。
**I 群は実機での目視で消化する**（①②とも 2026-09-21 にユーザーが確認済み）。
