...
| `supabase/scripts/judge-pending-spots.ts`（新規）+ `judgePending_test.ts`                                                    | S6       | 既存 pending への判定（既定は dry-run）                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| S6  | `chore: 既存の pending の寺社に公開の基準を当てるスクリプト`     | `supabase/scripts/judge-pending-spots.ts`（dry-run が既定、`--apply` で更新）。判定は `_shared/spotRules.ts` と research の関数を使い回す | Deno test（本番の実行は手順 H-5）                               |
...
- [ ] AC-42: `judge-pending-spots.ts` は引数なしだと dry-run で、各行の `id` `name` `判定（active / pending）` `落ちた基準` を出すだけで DB を書き換えない。`--apply` のときだけ `active` と判定した行を `status = 'active'`, `rank = 1` に更新する（Deno test。Supabase を注入）
