...
## 実機確認で変えたこと（2026-09-25）

- **D-4 を撤回**: 手がかりを「いまいる場所の近くの寺社の住所」から自動で作るのをやめ、最初は**全国から**探す。地域は本人が「地域を絞る」で入れたときだけ送る。東京の自宅で仙台大神宮を調べると「東京都 狛江市のあたり」になり、見つからなかった（家に帰ってから記録するのはよくある）。表示は「全国から探しています ・ 地域を絞る」/「{地域} を優先して探しています ・ 変える」。`nearbyHint` は削除
- プロンプト: 手がかりは同名の並べ順にだけ使い、その地域に無くても候補から外さない
- 候補が0件の理由を追えるよう、`spot_research_requests.diagnostics`（stop_reason・ブロックの種類と数・検索結果の数・検索のエラーコード・捨てた数。本文と手がかりは残さない）を足す（migration `20260925010000`）
- 本番に、昔の不具合（追加時の端末の位置を座標にしていた）で東京の座標のまま残っていた「鹿島台神社」の2行目があり、本人の pending が見えるようになったことで記録画面の自動選択に出た。記録は0件なので `merged` にする
