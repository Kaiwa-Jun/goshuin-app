...
- [x] AC-6（S3）: コピー完了後、`stamps.image_path` の全件について R2 に同じキーのオブジェクトがある（**DB の行数と一致**。Storage の一覧とは比べない — 一覧は消し残りを含むため）（2026-09-23 本番: DB 62 件 → コピー 62 / 失敗 0 / 残り 0。独自ドメイン経由で 62 件すべて 200。HEIC の中身の原本も width=400 の webp に変換されることを確認）
- [x] AC-7（S3）: R2 に `thumb-400/` または `view-1200/` を含むキーが1件も無い（2026-09-23: コピー前にバケットを空にし、コピーは原本の形のキーだけを対象にする。R2 への書き込みは他に無い）
