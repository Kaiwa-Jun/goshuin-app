...
## 追補4: ズームアウト上限によるクラッシュ経路の封じ込め（2026-08-02 方針決定）

### 経緯

追補3（現在地ピンの無限アニメ廃止）適用後も、全国スケール（delta ≥ 2）での連続操作（ズームアウト + 地方をまたぐスワイプ）でクラッシュが残存（イテレーション4 FAIL）。診断ログ上、落下直前まで JS ヒープ・描画数は健全で、アプリコード側の容疑（マーカー churn 3種・スナップショット機構・無限アニメリーク）は追補1〜3で排除済み。残る主容疑は Apple Maps 本体のタイルメモリ（全国表示でのパンは新地方のタイルを大量ロードし続ける最重量シナリオ）で、react-native-maps 越しに直接制御できない。

**ユーザー判断（2026-08-02）: 案A「ズームアウト上限で封じ込め」を採用**。全国スケールの恒久調査（実機クラッシュログの取得・Instruments でのネイティブ計測）は行わず、リリースを優先する。

### 対策

- `MapView` に **`minZoomLevel = 8`** を設定する（`MIN_ZOOM_LEVEL` 定数）。表示可能な最大幅は delta ≈ 360/2^8 = 1.41
- 根拠: 全クラッシュ実績は delta 2.28 以上（zoom 7 以下）、無事故で通過した実測済み領域は delta 1.33（zoom 8）まで。上限 zoom 8 は実測安全域の内側
- 制約: 日本全図の一覧表示は不可になる（地方単位までは表示可能）。宮城・東京の県フォーカス（fitToCoordinates）は現データの分布幅では影響なし。北海道など広域県の全域フィットは #67（全国展開）で再評価する
- `[#99diag]` の一時診断ログは調査終了に伴い削除する

### 追加する受入基準

- [ ] AC-48: `map-view` の `minZoomLevel` prop が `8` である（テストで props を直接検証）
- [ ] AC-49: `grep -r "#99diag" src/` が 0 件（一時診断ログの撤去）
- UI-1 の読み替え: 「日本全体が入るまでズームアウト」→「**ズーム可能な下限（minZoomLevel = 8）まで**ズームアウト → 初期ズームへ戻す、を連続20往復してもクラッシュしない。ズームアウトが地方スケールで止まることを確認する」

### スコープ外（追補4）

- 全国スケール表示の恒久解禁（実クラッシュログでの原因確定・Instruments 計測・Google Maps provider 検証を含む）→ feature-list に別エントリとして積み、Xcode 導入後に再開可能とする
