...
JavaScript とアセットだけの変更（UI の修正、文言、地図スタイル等）は、**ストア審査なしで配信できる**。

```bash
# 本番へ配信
npx eas-cli@latest update --branch production --message "バグ修正"
```

配信先は `eas.json` の各プロファイルの `channel` で決まる（production / preview / development）。

#### ⚠ どのビルドに届くか＝ runtimeVersion

`app.json` の `runtimeVersion` は **`{"policy": "fingerprint"}`** にしてある。ネイティブ側の構成をハッシュにした値が使われるので、**ネイティブが一致するビルドにしか更新が届かない**。

既定の `appVersion` 方針にしないのは、事故が起きるため。あれはアプリのバージョン文字列（1.0.0 など）を runtime version にするので、

1. ネイティブモジュールを足す（例: expo-image-manipulator）
2. バージョンを上げずに OTA を配信する
3. **そのモジュールを持たない既存ビルドに配信され、起動できなくなる**

fingerprint ならネイティブが変わった時点で別物として扱われるので、この経路が塞がる。

#### ⚠ 効き始めるのは「次のリリース」から

OTA を受け取れるのは **expo-updates を含むビルドだけ**。2026-09 に審査を通した build 15 には入っていないので、**そのビルドには一生 OTA が届かない**。次に審査を通したビルド以降で有効になる。

#### 疎通確認の結果（2026-09-21）

**実機（シミュレータ）で端から端まで通した。動く。**

1. `preview-simulator` プロファイルでビルド（fingerprint `fda7444…`）→ シミュレータに入れて起動
2. 検索欄の文言を一時的に書き換えて `eas update --branch preview-simulator`
3. アプリを2回起動し直す → **入れ直さずに文言が変わった**
4. 元に戻して再配信 → 元の文言に戻ることも確認（チャンネルは元の状態に戻してある）

要点:

- 配信された update の **runtime version がビルドの fingerprint と一致していること**が条件。ズレていると静かに無視される
- 反映は**2回目の起動**。1回目の起動で裏側にダウンロードし、次の起動で差し替わる。1回で変わらなくても壊れてはいない

#### 配信してよい範囲

Apple はアプリの本質を変えない範囲の更新を認めている。バグ修正・UI 調整・文言は問題ない。**機能の追加や課金導線の変更は審査に出すこと。**

**ネイティブの変更（モジュール追加、権限、app.json のネイティブ設定）は OTA では配れない。**ビルドし直して審査に出す。
