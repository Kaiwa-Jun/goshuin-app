...
**直近で完了（すべて 2026-08-09）**: 監査 A-4 アップロードエラー（#118 / PR #119）、グリッドの並び（PR #120）、監査 A-1 カメラが起動しない（PR #121・実機確認済み）、CI-1 の 404（PR #122）、P1-03 めくり UI が実機確認完了で `passes: true`、**P1-10 タブ入れ替え（案3）を develop にマージ（#123 / PR #124）**。

**次にやること（未定・要相談）**。監査の小さい指摘は**全部片付いた**（A-1〜A-14 と B-4 は完了 or 解決済み）。残っている候補:

- **残る設計判断**: D-3 記録の確認モーダル（→ P1-02 のブロッカー）/ D-5 ピンの色（⚠️ 要 `issue-099` 追補1〜4）/ D-6 受付時間のデータ整備（B-7）
- **バックログ**: P1-01 Maestro E2E の実戦検証 / P1-02 記録フロー最短化 / P1-04 パーソナル年報 / P1-07 地図の全国スケール解禁
- **バグ**: `ImageGalleryModal` のレンダー中 `setValue`（別 issue 化推奨）
- **CI-1 で見つけた別件**: `mention-response` のプロンプトインジェクション面 / `permissions` が3ジョブ共通で広い
- **期日**: 8/11 の cron 確認、App Store 審査結果、10/02 の Meta トークン
...
**feature-list で passes: false**: P1-01 Maestro E2E の実戦検証 / P1-02 記録フローの最短化（監査 B-1「3タップ」が実際は5〜6タップ・D-3 の判断待ち）/ P1-04 パーソナル年報 / P1-07 地図の全国スケール解禁。~~P1-03~~ は 2026-08-09 に実機確認完了で passes: true

**GitHub の open issue**: #82 買い切り型プレミアム（low）/ #67 全国ランク3マスタデータ（medium）/ #47 訪問ルートの記録・共有

**設計判断が要るもの**: B-7 受付時間は仕組みだけあってデータが無い（D-6）/ D-3 記録の確認モーダル（P1-02 のブロッカー）/ D-5 ピンの色（要 #099 追補）。~~タブ入れ替え~~ / ~~B-6~~ は P1-10 で、~~B-4~~ は PR #127 で、~~D-4~~ は「御朱印帳の中だけ」で決定済み
