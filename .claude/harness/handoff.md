...
**D-5 と D-6 はどちらも完了し、実機確認も済んでいる。次の一手は下の「次にやること」の1番から。**

⚠️ **残っているのは Issue #132 の実機チェックだけ**（次に参拝したとき / 地図を触るときにまとめて消化する）。P1-02 の位置情報2項目 + D-5 の「行きたいピンとの判別」+ D-6 の受付時間表示。

審査（build 13）は**結果待ちで、こちらからやることは無い**（8/11 深夜時点で `WAITING_FOR_REVIEW`・Apple からの新着メールも無し）。

### 次にやること（優先順）

1. **P1-01 Maestro E2E**（中）— `smoke.yaml` は更新済みだが**一度も走らせていない**。CI にも e2e ジョブが1つも無い。⚠️ **シミュレータのアプリは 8/2 ビルドなので、実走の前に dev client の作り直しが要る**（下記）
2. **小物**: `ImageGalleryModal` のレンダー中 `setValue`（`src/components/common/ImageGalleryModal.tsx:79-88`・**issue 化すらまだ**）/ `mention-response` のプロンプトインジェクション面 + `permissions` の縮小 / `docs/design/ui-design.md` のピン表示ルール表に残る古い記述（サイズ欄と現在地のパルス。実装と乖離）
3. **大きいもの**: P1-04 パーソナル年報 / P1-07 地図の全国スケール解禁（実機 `.ips` + Instruments）/ P2-02 記事単位クロール
4. **技術的負債**: migration 履歴の乖離（下記 `db push` の節）/ Cloudflare トンネルの URL 固定化 / `task-board.html` が 8/9 で止まったまま（作り直すか廃止するか未決）
