# セッション引き継ぎ（最終更新: 2026-08-11 深夜）

## ▶ 再開したらここから（2026-08-11 深夜 時点）

**朝いちの判断は2つ。どちらも5分で終わる。**

1. **PR #141（D-5 未訪問ピンの色）をマージするか** — 実装・検証・実機確認まで済んでいて、**マージだけ残っている**。夜間は勝手にマージしない方針にしたので止めてある
2. **D-6 を案A/B/C のどれで進めるか** — 測定は終わった。**取得率 46%（explicit のみなら 29%）で、rank5 全件展開は推奨しない**。報告書 `docs/project/reception-hours-yield-2026-08.md` の §6 に3案と推奨（**案B**）がある

審査（build 13）は**結果待ちで、こちらからやることは無い**（8/11 深夜時点で `WAITING_FOR_REVIEW`・Apple からの新着メールも無し）。

### 次にやること（優先順）

1. **PR #141 のマージ**（D-5 / Issue #140）— 上記
2. **D-6 の案の選択**（上記）。案Bなら `supabase/seeds/seed_reception_hours_2026-08.sql` を SQL Editor で流す → 実機で表示確認 → `RecordScreen.tsx:272-275` のメモガイドから「受付時間」を外す、で完了
3. **P1-01 Maestro E2E**（中）— `smoke.yaml` は更新済みだが**一度も走らせていない**。CI にも e2e ジョブが1つも無い。まずシミュレータでローカル実走 → CI 組み込みは別判断（macOS ランナーが要るため重い）。⚠️ **シミュレータのアプリは 8/2 ビルドなので、実走の前に dev client の作り直しが要る**（下記）
4. **小物**: `ImageGalleryModal` のレンダー中 `setValue`（`src/components/common/ImageGalleryModal.tsx:79-88`・**issue 化すらまだ**）/ `mention-response` のプロンプトインジェクション面 + `permissions` の縮小
5. **大きいもの**: P1-04 パーソナル年報 / P1-07 地図の全国スケール解禁（実機 `.ips` + Instruments）/ P2-02 記事単位クロール
6. **技術的負債**: migration 履歴の乖離（下記 `db push` の節）/ Cloudflare トンネルの URL 固定化 / `task-board.html` が 8/9 で止まったまま（作り直すか廃止するか未決）

### 夜間に進めたこと（2026-08-11 深夜・オートモード）

|                            | 内容                                                                                                                                                                   |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **D-5 実装完了**           | Issue #140 / **PR #141（マージ待ち）**。`pin.unvisited` を `primary[200]` (`#FED7AA`) に。テスト1129件全パス・lint 0・typecheck clean。**ユーザーが実機で確認して OK** |
| **D-6 段階1 完了**         | 28スポットを実測。**取得率 46%（13/28）／explicit のみ 29%（8/28）**。報告書 + seed SQL ドラフト（**未実行**）。**rank5 全件展開は推奨しない**                         |
| **シミュレータの前提崩れ** | `goshuin-shot` のアプリは **8/2 ビルドの埋め込みバンドル版で Metro に繋がらない**と判明（下記）                                                                        |

### ⚠️ シミュレータ `goshuin-shot` は「いまの develop」を映さない（2026-08-11 深夜に判明）

D-5 の見た目を確認しようとして発覚。**インストールされているのは 8/2 ビルドで `main.jsbundle` を同梱しており、dev client のフレームワークが無い＝ Metro に繋がらない。** タブが旧構成（御朱印 / コレクション / 設定）のままなのが証拠。

引き継ぎに書いてあった「dev client は Metro から作業ツリーを読むため撮れるのは常にいまの develop」は、**実機の Dev Client の話で、このシミュレータには当てはまらなかった**。

📌 **波及: ストアスクショをシミュレータで撮り直す計画は、このままだと 8/2 の見た目が撮れる。** P1-01 の Maestro 実走も同じ制約を受ける。直すには `eas build --profile development-simulator --platform ios` で作り直しが要る（**未実施・要判断**）。実機の Dev Client（トンネル経由）は従来どおり作業ツリーを読むので、そちらは影響なし。

### D-6 段階1 の結果（要点。全文は `docs/project/reception-hours-yield-2026-08.md`）

**取得率 46%（13/28）。御朱印と明記されたものに限ると 29%（8/28）。** 判断基準の下限側。

⚠️ 対象は「29スポット」ではなく**公式サイト URL を持つ28スポット**だった（1件は `sns_link` のみ）。

**この数字を悪い方に補正する4つの所見**:

1. **この28件は URL 既知の最良サンプル**。残り441件はサイト探しから始まるので歩留まりはこれより下がる
2. **御朱印の専用ページがあっても時間が書いていない**（東寺 `/nokyosho/`・大崎八幡宮 `/gosyuin/`・志波彦 `/about/goshuin.html`・烏森 `/御朱印` の4件）。「ページを見つければ取れる」は成り立たない
3. **主要スポットほど記載が無い**（伏見稲荷・清水寺・金閣寺・上賀茂・下鴨・靖國・神田・増上寺）。利用者が最も行く先ほど埋まらない
4. **proxy（社務所・授与所の時間）は実際より広い時間を出す危険**。湯島天満宮は社務所 19:00 だが授与は 17:00 終了

**実装面の発見（どちらもコード変更が要らないと分かった）**:

- ✅ **取得日の併記は実装不要**。`SpotInfoSection.tsx:26-33` が `info_data.notes` を「受付 9:00〜17:00（notes）」の形で既に描画する。型（`types/supabase.ts:36`）も `notes` を許容済み。**D-6 の着手手順3は消化済み**
- ✅ **seed 行はメモに消されない**。`extract-spot-info` は UPSERT のみで delete が1箇所も無い

**seed するときの落とし穴2件（seed SQL では対処済み）**:

- ⚠️ **スポット名は一意ではない**（八坂神社2件・日枝神社3件）。名前で引く SQL を書くと別スポットに受付時間が付く → UUID 直書きにしてある
- ⚠️ **宮城縣護國神社は既にメモ由来の行があり、値も食い違う**（メモ 17:00 / 公式 16:00）。D-6 は「メモが勝つ」なので `on conflict ... where source_stamp_ids = '{}'` でメモ由来の行を更新対象から外してある

### 今日やったこと（2026-08-11）

|                            | 内容                                                                                                                                                                                                                                                                                                |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **審査対応（午前〜夕方）** | PR #135（5.1.1(iv) 位置情報プロンプト）/ PR #136（5.1.1(v) アカウント削除・Issue #134）/ PR #137（develop → main・152コミット）。Edge Function `delete-account` を本番デプロイし 401 を4パターンで実証、migration `20260811000000` 適用、実機 G-1〜G-4 全確認、**build 13 を 17:22 に審査へ再提出** |
| **PR #138（Issue #133）**  | `fetchVisitedSpotIds` の握り潰しを解消。空 Set をやめて throw し、失敗時は件数もバッジも算出せず完了画面に注記を出す。**マージ済み**（merge `f547d1d`）。⚠️ 実機回帰 F-1〜F-4 のみ未消化                                                                                                            |
| **Issue #139 / P1-11**     | オンボーディング作り直しに着手 → **同日ユーザー判断で中断**。下記「P1-11 の中断」参照                                                                                                                                                                                                               |
| **設計判断 D-5 / D-6**     | **両方決定**。記録は `docs/design/ux-audit-2026-08.md` 末尾                                                                                                                                                                                                                                         |

Guideline 2.1(a)（カメラ無反応）は **PR #121（8/9）で修正済み**で、build 12 に入っていなかっただけ。コード作業は不要だった。

### D-6 で判明した、前提が誤っていたこと

⚠️ **rank5+4 は 949件**（本番実測: rank5=469 / rank4=480 / rank3=103 / rank2=19 / rank1=38）。**「29スポット」はクローラの巡回対象（東京・宮城・京都の rank5）の話**で、全国の rank5 とは別物だった。公式サイト URL を持っているのもその29件だけで、残りはサイト探しから始まる。**だから段階実行にした。**

⚠️ **受付時間のパイプラインは end-to-end で実際に動いている。** 未実装でも故障でもない。`useRecordForm.ts:165` → `extract-spot-info` がメモ本文を Claude に渡す → `spot_aggregated_info` に集約 → `SpotInfoSection` が描画。**データが2件しかないのは入力がユーザーの手書きメモだから**で、ユーザーが実質いないため供給が立ち上がっていない。監査 B-7 はこの原因まで書いていなかった。

⚠️ **seed とメモの優先順位はコード変更不要。** `extract-spot-info/index.ts:220-223` の受付時間の集約が既に "Latest wins" なので、「seed を置き、メモが来たら上書き」が既存実装のまま成立する。

### ⚠️ ASC の画面操作は Claude に代行させない（2026-08-11 に判明）

今日いちばん時間を溶かした。**ログイン・添付・ビルド差し替え・提出はユーザーが手でやるのが速い。** Claude は文面作成と DB / CLI 側に徹すること。

- **返信ダイアログの「ファイルを添付」が機能しない**。file input に `change` が正しく届いていても ASC が即クリアする。**動画でも 23KB の JPEG でも同じなので形式の問題ではない**（一度「MP4 を弾いている」と誤診した）
- **App Review 情報の添付は通る**が、**アップロード完了時にフォームが再取得され、未保存の編集（ビルド変更・メモ追記）が巻き戻る**
- その後**ビルド一覧が読み込まれなくなり回復しなかった**。ページを離れようとしても beforeunload ダイアログが出て Claude 側からは破棄できない
- 返信文のドラフトは `~/Downloads/asc-reply-build13.txt`。次回もここを土台にする
...
### ⏸ P1-11 オンボーディング — 中断中（2026-08-11）

**ブロッカーは絵のテイストだけ。** それ以外は決まっていて、ブランチ上に実装もある。

|          |                                                                                                                                                                    |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ブランチ | `feature/issue-139-onboarding-visual-redesign`（**9コミット・未 push**・作業ツリーはクリーン）                                                                     |
| Issue    | **#139**（open）                                                                                                                                                   |
| 契約書   | `docs/issues/issue-139-onboarding-visual-redesign.md`（**追補1まで**。本文は4スライド前提で、追補1が3スライドに上書きしている）                                    |
| 参考     | `docs/design/onboarding-illustration-references.md`（テイスト候補）/ `onboarding-stitch-brief.md`（Stitch 用）/ `onboarding-art/`（unDraw のオレンジ置換済み素材） |

**中断の理由はデザインの質で、実装や検証の不足ではない。** goshuin-evaluator は2回目で **PASS 66/66**、テスト 1169件・lint 0 errors・typecheck clean。それでもユーザーが「デザイン的にダメ」と判断した。その後 Stitch でも試作 → 不採用、unDraw の3点も「微妙」。

**決まっているもの**: スライド3枚 / 各枚が1つのメリットを伝える / 文言（追補1の表）/ 地は `primary[50]` / `colors.illustration` トークン / `OnboardingIllustration` の構造 / **G 群8項目のテスト**

**残っているもの**: 絵のテイストだけ

⚠️ **再開時に必ず読むこと**: Guideline 5.1.1(iv)（PR #135）。**最終スライドの脱出口は「続ける」1つだけ**で、権限リクエストを回避できる導線を置くと再却下される。この制約を固定するテスト8件はブランチ上に既にある。

⚠️ **教訓**: 受入基準を全部満たしてもデザインが良いとは限らない。**見た目が主題の Issue では、実装前にテイストの実物合意を取ること。** 当たりのモックを自分で描いて見せるのは方向の議論には使えるが、採用可否の判断材料にはならない。

📌 副産物: **`react-native-svg` は未導入**（推移的依存にも無い）。ベクター描画が要る機能は dev build の作り直しを伴う。

### 同じタイミングで消化する: Issue #133 の実機回帰（F-1〜F-4）

PR #138 の未検証分。**どれも正常系が壊れていないことの確認だけ**（失敗経路は一時的な通信の揺らぎでしか起きず実機で再現できないため、ユニットテスト13件で担保して実機は回帰のみと決めた）。

1. オンラインで記録 → 完了画面に従来どおり `N箇所目の御朱印！` が出る
2. 1 で「通信エラーのため記録数を表示できません」の注記が**出ない**
3. 件数が閾値（1 / 5 / 10 / 30 / 50 / 100）に達したときバッジアニメーションが出る（**実記録数が閾値-1 のときしか見られない**ので、達しなければ未検証のままで可）
4. 地図タブで訪問済みスポットのピンが従来どおり訪問済み表示になる
...
**次にやることは先頭の節が正。** ここには置かない（重複すると片方が腐る）。

**GitHub の open issue（2026-08-11 深夜 時点）**: **#140 未訪問ピンの色（→ PR #141 マージ待ち）** / **#139 オンボーディング（⏸中断中）** / **#132 P1-02 の実機未検証2項目（参拝時に確認。⚠️ D-5 の D-3 もここで一緒に見る）** / #82 買い切りプレミアム（low）/ #67 全国 rank3 マスタデータ（medium）/ #47 訪問ルートの記録・共有。~~#133~~ は PR #138 で解決済み。

**open PR**: **#141**（D-5 未訪問ピンの色・**マージ待ち**）
...
📌 **ストアスクショの方針（2026-08-09 決定）**: 残り3枚を先に撮るのはやめた。理由: ①ストアには既に6枚アップロード済みで**審査はそれで進んでおり、残り3枚はブロッカーではない** ②dev client は Metro から作業ツリーを読むため撮れるのは常に「いまの develop」で、審査中の build 12（8/8 早朝・#114 も #116 も入っていない）とは一致しない。**次のビルド提出とセットで6枚とも撮り直す**（build 13 を出したので、いまなら撮り直せる。ただし審査はスクショ6枚で進んでいるのでブロッカーではない）。シミュレータ `goshuin-shot` にアプリはインストール済み・Maestro も動くので、**ログインさえ通れば撮影は自動化できる**（AsyncStorage に保存セッションが無いことを確認済み。シミュレータ上で1回ログインしてもらえば以降は自動）
...
⚠️ **このボードは 2026-08-09 で止まっており、もはや正ではない**（8/11 の審査対応・PR #138・Issue #139・D-5/D-6 の決定が未反映）。作り直すか廃止するかは未決。**一覧の正はこの handoff と `feature-list.json` と GitHub の open issue**（Artifact: https://claude.ai/code/artifact/961421e9-0005-4116-9b18-f3856646e0aa ）。全31件 + ユーザー作業7件を性質別（バグ / 小さい改善 / 設計判断 / バックログ / リリース・期日 / ユーザー作業）に束ね、規模・根拠の file:line・ブロッカー・期日を載せてある。**チェックは Claude が更新する運用**（完了したら該当項目に `done` クラスを付け、セクションと rail のカウントも直して同じ URL に再デプロイする。`file_path` を同じにすれば URL は変わらない）。
...
**設計判断が要るもの**: ~~D-5 ピンの色~~ / ~~D-6 受付時間~~ は **2026-08-11 に両方決定**（`docs/design/ux-audit-2026-08.md` 末尾の「D-5 の決定」「D-6 の決定」）。~~タブ入れ替え~~ / ~~B-6~~ は P1-10 で、~~B-4~~ は PR #127 で、~~D-4~~ は「御朱印帳の中だけ」で、~~D-3 確認モーダル~~ は「廃止して完了画面に取り消し」で決定済み（PR #131）
...
- **App Store 審査結果**（build 13 を 8/11 17:22 提出・ステータス「審査待ち」）。⚠️ **build 11 は提出 8/2 → 却下 8/6 で約4日**かかっているので 48時間で焦らなくてよい。**通過したら手動リリースの操作が要る**（リリース設定が「このバージョンを手動でリリースする」）
...
- **審査結果が来たら対応**（通過 → ASC で手動リリース / 却下 → 内容を確認して対応）。要 Apple ログイン
- **`.p8` のオフマシン退避** — ✅ 2026-08-11 にユーザーがパスワードマネージャへ保存済み。ローカルの正は `~/.appstoreconnect/`
...
- ✅ **buildNumber 12 を 2026-08-08 に再提出 → 2026-08-10 に3件で却下**（5.1.1(v) アカウント削除なし / 2.1(a) カメラ無反応 / 5.1.1(iv) 位置情報プロンプト）。→ **build 13 で全件対応して 2026-08-11 17:22 に再提出**（経緯は先頭の節）
...
御朱印アプリ（Expo + Supabase）の **Phase 0（iOS 先行リリース）は大詰め**。**v1.0.0 build 13 が審査待ち**（3回目の提出）。Android は iOS 優先のため保留。ここからは Phase 1（記録体験の磨き込み）と、差別化の本丸である限定御朱印情報の機能に着手するフェーズ。開発は `/build-feature` の自律ループ（契約書 → TDD → 機械検証 → goshuin-evaluator → 人間ゲート → PR）で回す。**人間ゲートは push/PR 直前の1箇所のみ**。

## リリース状況（2026-08-11 時点）

- **アプリ名: 御朱印さんぽ**（「御朱印コレクション」→「御朱印マップ」と変遷。マップは App Store で登録済みだったため。45競合を調査して決定）
- **iOS: 審査中（3回目）**。v1.0.0 / **buildNumber 13** / ascAppId `6797201465` / Apple Team `292ZWTG3UD`。却下の履歴: build 11 → 5.1.1 マイク権限プレースホルダー（PR #113 で修正）/ build 12 → 5.1.1(v) アカウント削除なし・2.1(a) カメラ無反応・5.1.1(iv) 位置情報プロンプト（PR #121 / #135 / #136 で修正）。**build 13 は 8/11 17:22 提出で審査待ち**
...
**先頭の「▶ 再開したらここから → 次にやること」が正。** ここは重複するので置かない。

## ユーザー待ちの項目

- **審査結果への対応**（通過 → 手動リリース / 却下 → 内容確認）。要 Apple ログイン
- **ストアスクショ6枚の撮り直し** — build 13 の見た目に合わせて。審査通過後でも可
- **Issue #132: 次に参拝したときの実機確認2項目**（先頭の節を参照）
- ~~**Google Play**~~ → **iOS 優先のため保留**（2026-08-11 ユーザー判断）
- **App Store Connect の操作** — ⚠️ **Claude に代行させないこと**。Chrome 拡張からログインはできる（ユーザーが2FAまで通せば）が、添付とビルド差し替えが壊れる。詳細は先頭の「ASC の画面操作は Claude に代行させない」を参照
