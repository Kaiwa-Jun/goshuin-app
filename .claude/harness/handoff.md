...
**D-5 と D-6 はどちらも完了した。次の一手は下の「次にやること」の1番から。**

審査（build 13）は**結果待ちで、こちらからやることは無い**（8/11 深夜時点で `WAITING_FOR_REVIEW`・Apple からの新着メールも無し）。

### 次にやること（優先順）

1. **実機で受付時間の表示を見る**（D-6 の唯一の残件・数分）— スポット詳細に「受付 9:00〜17:00（2026年8月時点／公式サイト）」が出るか。⚠️ **明治神宮と瑞巌寺は「受付 時間情報あり（…）」になるのが正しい**（終了時刻が季節・月で変わるため `close` を置いていない）
2. **P1-01 Maestro E2E**（中）— `smoke.yaml` は更新済みだが**一度も走らせていない**。CI にも e2e ジョブが1つも無い。⚠️ **シミュレータのアプリは 8/2 ビルドなので、実走の前に dev client の作り直しが要る**（下記）
3. **小物**: `ImageGalleryModal` のレンダー中 `setValue`（`src/components/common/ImageGalleryModal.tsx:79-88`・**issue 化すらまだ**）/ `mention-response` のプロンプトインジェクション面 + `permissions` の縮小 / `docs/design/ui-design.md` のピン表示ルール表に残る古い記述（サイズ欄と現在地のパルス。実装と乖離）
4. **大きいもの**: P1-04 パーソナル年報 / P1-07 地図の全国スケール解禁（実機 `.ips` + Instruments）/ P2-02 記事単位クロール
5. **技術的負債**: migration 履歴の乖離（下記 `db push` の節）/ Cloudflare トンネルの URL 固定化 / `task-board.html` が 8/9 で止まったまま（作り直すか廃止するか未決）

### 夜間に進めたこと（2026-08-11 深夜・オートモード）

**D-5 / D-6 とも完了。PR #141 / #143 / #144 はすべてマージ済み。**

|                            | 内容                                                                                                                                                                                                         |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **D-5 完了**               | Issue #140 / **PR #141 マージ済み**。`pin.unvisited` を `primary[200]` (`#FED7AA`) に。ユーザーが実機で確認して OK。⚠️ **D-3 のみ未確認**（Issue #132 に追記済み）                                           |
| **D-6 完了（案B）**        | Issue #142 / **PR #143・#144 マージ済み**。取得率 46%（13/28）だったため**全件展開はしない**と決定。**seed を本番投入済み**（12件中11件・1件はメモ優先で正しくスキップ）。メモガイドから「受付時間」を外した |
...
**GitHub の open issue（2026-08-11 深夜 時点）**: **#139 オンボーディング（⏸中断中）** / **#132 P1-02 の実機未検証2項目（参拝時に確認。⚠️ D-5 の D-3 もここで一緒に見る）** / #82 買い切りプレミアム（low）/ #67 全国 rank3 マスタデータ（medium）/ #47 訪問ルートの記録・共有。~~#133~~ は PR #138 で解決済み。

**open PR**: なし。~~#141~~（D-5）/ ~~#143~~（D-6 測定）/ ~~#144~~（D-6 メモガイド）はすべてマージ済み。~~#140~~ / ~~#142~~ もクローズ済み。
