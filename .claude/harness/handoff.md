...
棚卸しはしたが**着手対象は未決定**。ユーザーは「一覧を見てから決めたい」と言っており、`task-board.html` の作り直しを提案したところで `/clear` になった。

1. **Issue #133**（規模: 小・**推奨**）— `fetchVisitedSpotIds` が通信エラーで空 Set を返し、記録完了画面の「N箇所目」がずれて獲得済みバッジが再発火する。記録というコア導線の上にあるバグ。監査 B-3（エラーの握り潰し）への最初の一手にもなる
