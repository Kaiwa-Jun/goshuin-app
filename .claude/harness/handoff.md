...
- ✅ **FAB を右下へ移動（実装済み・未コミット）**: `MapScreen.tsx` の `fabContainer` を `alignSelf:'center'` → `right:20`。typecheck 通過・MapScreen テスト51件パス。**タブ構成とは独立した判断**として先行対応した
