...
- タブが旧構成（**御朱印 / コレクション / 設定** ← いまは 御朱印帳 / あつめる / 自分）
- ボトムシートが #114 の再設計前、行きたいアイコンが旗（いまはブックマーク）
- 地図のピンがグレー（D-5 で淡いオレンジに変更済み）

**Apple はこのアプリを4回見ているので、次に 2.3.3 を指摘される可能性はある。** ただし3回の審査で一度も指摘されていないためブロッカーではない。撮り直すなら build 14 とセット。⚠️ **シミュレータのアプリは 8/2 ビルドなので現状のままでは撮り直せない**（下記）。実機で撮るか、`eas build --profile development-simulator` で作り直すかの二択。
