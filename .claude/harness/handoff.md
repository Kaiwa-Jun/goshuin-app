...
**監査（A-1〜A-14 + B-4）はこれで全部片付いた**。P1-03 / P1-10 とも実機確認済みで `passes: true`。テストは 1002 → **1052件**。

**次にやること: D-3（記録の確認モーダルを残すか）を決めて P1-02（記録フローの最短化）へ。**

`docs/design/ux-audit-2026-08.md` B-1 のとおり、コアバリュー「3タップで記録完了」が実際は5〜6タップ。最後の `ConfirmModal` が1タップ分を占めている。**論点は「確認モーダルを廃して記録後に Undo できる方式にするか」**（誤登録防止 vs コアバリュー）。ここが決まらないと P1-02 の契約書が書けない。

⚠️ 8/9 に日付ピッカーまわりを2回触っている（PR #127 の和暦併記、PR #129 の spinner 化）ので、記録画面の現状はコードを読んで確認すること。
...
- **GitHub open issue**: #82 買い切りプレミアム（low）/ #67 全国 rank3 マスタデータ（medium）/ #47 訪問ルートの記録・共有
