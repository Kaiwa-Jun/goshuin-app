...
## ▶ 再開したらここから（2026-08-11 時点）

**いまの状況: App Store 審査 3回目の対応を全部終えて、結果待ち。** コード・インフラ・実機検証・提出まで完了しているので、**次セッションで審査のためにやることは無い**（結果が来たら動く）。

### 今日やったこと（2026-08-11）

|             | 内容                                                                                                                          |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **PR #135** | Guideline 5.1.1(iv) 位置情報の事前プロンプト。「続ける」1本に絞り、権限リクエストを回避する導線を撤去                         |
| **PR #136** | Guideline 5.1.1(v) **アカウント削除機能**（Issue #134・契約書 `docs/issues/issue-134-account-deletion.md`）                   |
| **PR #137** | develop → main のリリースマージ（152コミット）                                                                                |
| 本番        | Edge Function `delete-account` デプロイ + 401 を4パターンで実証 / migration `20260811000000` 適用（`confdeltype` a→n を確認） |
| 実機        | **G-1〜G-4 すべて確認**。1枚記録 → 削除 → 孤児0件・画像も消滅 → 再ログインで別 UUID の空アカウント                            |
| ASC         | build 13 を submit → 録画を添付して3件まとめて返信（17:05）→ build 13 を紐付けて**審査へ再提出（17:22・審査待ち）**           |

Guideline 2.1(a)（カメラ無反応）は **PR #121（8/9）で修正済み**で、build 12 に入っていなかっただけ。コード作業は不要だった。

### 次にやること（優先順・ユーザーと未合意）

⚠️ **1件目の Issue #133 は 2026-08-11 に完了（PR #138）。** 以下の 2 以降が残り。着手対象は未決定で、`task-board.html` の作り直しはユーザー判断で**後回し**にしてある（8/9 で止まったまま）。

1. ~~**Issue #133**~~ — **完了（PR #138・merge `f547d1d`）**。⚠️ **実機回帰 F-1〜F-4 だけ未消化**（下記「次に参拝したときに必ず確認すること」に統合済み）
2. **P1-01 Maestro E2E**（中）— 今日 `e2e/flows/smoke.yaml` を直したが**一度も走らせていない**。シミュレータで通して CI へ
3. ~~**P1-11 オンボーディングの見た目**~~ — **⏸ 着手したが中断（2026-08-11・ユーザー判断）**。下記「P1-11 の中断」参照
4. **P2-02 第2柱**（中）公式サイトの記事単位クロール / **P1-04**（大）パーソナル年報 / **P1-07**（大）地図の全国スケール解禁
5. **設計判断が要るもの**: D-5 ピンの色（⚠️ `docs/issues/issue-099-map-clustering.md` 追補1〜4 必読）/ D-6 受付時間のデータ（全1,109スポット中2件のみ）
6. **技術的負債**: migration 履歴の乖離（下記の `db push` の節）/ Cloudflare トンネルの URL 固定化（名前付きトンネル）/ `mention-response` のプロンプトインジェクション面
...
⚠️ **このボードは 2026-08-09 で止まっている**（8/11 の審査対応と Issue #133 / P1-11 が未反映）。作り直しをユーザーに提案したが未着手。**一覧の正はタスクボード**（Artifact: https://claude.ai/code/artifact/961421e9-0005-4116-9b18-f3856646e0aa ）。全31件 + ユーザー作業7件を性質別（バグ / 小さい改善 / 設計判断 / バックログ / リリース・期日 / ユーザー作業）に束ね、規模・根拠の file:line・ブロッカー・期日を載せてある。**チェックは Claude が更新する運用**（完了したら該当項目に `done` クラスを付け、セクションと rail のカウントも直して同じ URL に再デプロイする。`file_path` を同じにすれば URL は変わらない）。
