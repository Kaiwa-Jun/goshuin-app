...
**次にやること: P1-10 の実機確認 N 群6項目。** タブ入れ替えは develop に入ったが `passes` は **false のまま**。

- タブバーで「御朱印帳」がはみ出さないか（4文字）
- 行きたいカードのタップで地図が該当スポットへ動き、ボトムシートが開くか
- 行きたいから削除 → 一覧から消え、地図のピンの色も変わるか
- 自分タブの位置情報の行から OS 設定アプリへ飛ぶか
- OS 設定で許可/拒否を変えてアプリに戻ると、行の表示が追随するか
- 小型端末でタブバーのラベルが省略されないか

**その次の候補**: 小さい改善を1 PR に束ねる（A-2 / A-3 / A-10 / A-11 + B-4）。⚠️ **A-13 と A-14 は P1-10 で解決済みなので束から外す**。A-2 は D-4（和暦をどこまで出すか）の判断待ち。
...
**設計判断が要るもの**: 監査 B-4（ui-design v6 が撤回済み方針のまま。⚠️ **タブ名も v6 の記述と食い違うようになった**）/ B-7 受付時間は仕組みだけあってデータが無い / D-3 記録の確認モーダル（P1-02 のブロッカー）/ D-4 和暦のスコープ（A-2 のブロッカー）/ D-5 ピンの色（要 #099 追補）。~~タブ入れ替え~~ と ~~B-6~~ は P1-10 で解決済み
