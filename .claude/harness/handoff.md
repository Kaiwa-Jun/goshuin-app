...
**直近で完了**: 監査 A-4 アップロードエラー（#118 / PR #119）、グリッドの並び（PR #120）、監査 A-1 カメラが起動しない（PR #121・実機確認済み）、CI-1 の 404（PR #122）、**Issue #116 = P1-03 めくり UI が実機確認完了で `passes: true`（2026-08-09）**。

**次にやること: タブ入れ替え（案3: 地図 / 御朱印帳 / あつめる / 自分）。** P1-03 が `passes: true` になったのでブロッカーは外れた。行きたいリストを地図タブへ移す。規模 L なので契約書から。

**先に片付けるなら**: 小さい改善を1 PR に束ねる（A-2 / A-3 / A-10 / A-11 / A-13 / A-14 + B-4）。⚠️ ただし **A-10 と A-13 は案3で動く画面**なので、案3を先にやるなら後回しでよい。A-2 は D-4（和暦をどこまで出すか）の判断待ち。

**#116 の実機 N 群8項目は全消化（2026-08-09）**。2回に分けて確認した:

- 1回目: グリッドの並び関連 → 「左上が一番古い」に直した（PR #120）。めくりが壊れていないこと・拡大スワイプの向きも確認
- 2回目: スワイプの手触り / スナップ・小型端末での収まり・再起動をまたぐ表示モード永続化（`AsyncStorage` の `gallery_view_mode`）・白紙ページ（末尾＝一番左）タップで記録画面・1件入った状態のページ番号と和暦 → **すべて OK**
