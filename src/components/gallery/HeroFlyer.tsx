 ...
 * 写真が出てくるのを待つ上限。
 *
 * 飛ぶ1枚は新しい <Image> なので、一覧に同じ写真が出ていても、その View に
 * とっては初めての読み込みになる。実測で速いときは 19ms、一覧の読み込みが
 * 立て込んでいると 200ms を超える。短く切ると、間に合わなかったときだけ
 * 灰色の枠が出て「押すとちらつく」ことになるので、長めに待つ。
 * 待っている間は何も描かないので、伸びても画面は静かなまま
 */
        ...
        {flight && target && (
          <Animated.View
            style={[
              styles.box,
              {
                // 写真が出せるまで見せない。見せるとタイルの上に灰色が乗って、
                // 押した瞬間にちらついて見える（初回だけ起きるのはこれ）。
                // ⚠ 描画自体は続ける。外すと中の <Image> が読み込まれず、
                //   いつまでも出せる状態にならない
