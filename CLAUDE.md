...
0. **ブランチ作成**: `develop` から `feature/issue-{番号}-{概要}` ブランチを切る
1. **Red**: 失敗するテストを書く
...
```
---
commit & push & PR 作成が完了しました。

PR: （PR の URL）

次のステップ:
- PR をマージする場合は「マージして」と指示してください
...
- **develop**: 開発統合ブランチ。feature ブランチからの PR をマージする先
- **feature/issue-{番号}-{概要}**: 各 Issue の実装用ブランチ。develop から切る

```
main ← develop ← feature/issue-XXX-yyyy
```
